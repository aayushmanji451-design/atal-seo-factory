<?php
/**
 * Build both standalone Task 05 development plugins.
 *
 ...
 *
 * @param string $project_root Project root.
 ...
 */
function build_task05_plugin( string $project_root, string $release_dir, string $slug, string $filename ): array {
	...
	$sources  = array(
		...
		$project_root . '/packages/seo-images/src' => $slug . '/dependencies/seo-images',
	...
	if ( file_exists( $zip_path ) && ! unlink( $zip_path ) ) {
		throw new RuntimeException( 'Unable to replace the Task 05 development ZIP.' );
	}
	$zip = new ZipArchive();
	if ( true !== $zip->open( $zip_path, ZipArchive::CREATE | ZipArchive::EXCL ) ) {
		throw new RuntimeException( 'Unable to create the Task 05 development ZIP.' );
	}
	...
	if ( ! $zip->close() ) {
		throw new RuntimeException( 'Unable to finalize the Task 05 development ZIP.' );
	}
	...
	if ( false === $hash || false === file_put_contents( $zip_path . '.sha256', strtoupper( $hash ) . '  ' . basename( $zip_path ) . PHP_EOL ) ) {
		throw new RuntimeException( 'Unable to write the Task 05 checksum.' );
	}
	return array(
		'zip'    => $zip_path,
		'files'  => count( $entries ),
...
$builds = array(
	build_task05_plugin( $project_root, $release_dir, 'atal-seo-factory-core', 'atal-seo-factory-core-0.5.0-dev-task-05.zip' ),
	build_task05_plugin( $project_root, $release_dir, 'atal-diploma-receiver', 'atal-diploma-receiver-0.5.0-dev-task-05.zip' ),
);
