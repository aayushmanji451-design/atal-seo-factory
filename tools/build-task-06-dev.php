<?php
/**
 * Build the standalone Task 06 Core development plugin.
 *
...
$zip_path     = $release_dir . '/atal-seo-factory-core-0.6.0-dev-task-06.zip';
$sources      = array(
	...
	$project_root . '/packages/topics/src'     => $slug . '/dependencies/topics',
...
if ( file_exists( $zip_path ) && ! unlink( $zip_path ) ) {
	throw new RuntimeException( 'Unable to replace the Task 06 development ZIP.' );
}
$zip = new ZipArchive();
if ( true !== $zip->open( $zip_path, ZipArchive::CREATE | ZipArchive::EXCL ) ) {
	throw new RuntimeException( 'Unable to create the Task 06 development ZIP.' );
}
...
if ( ! $zip->close() ) {
	throw new RuntimeException( 'Unable to finalize the Task 06 development ZIP.' );
}
...
if ( false === $hash || false === file_put_contents( $zip_path . '.sha256', strtoupper( $hash ) . '  ' . basename( $zip_path ) . PHP_EOL ) ) {
	throw new RuntimeException( 'Unable to write the Task 06 checksum.' );
}
